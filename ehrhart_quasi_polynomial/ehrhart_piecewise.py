...
class PiecewiseEhrhartQuasiPolynomial():
    def __init__(self, A):
        self.A = A
        self.sec_fan = self._secondary_fan()

        self._projection_generator()
        self.max_cones, self.qps, R, S = self._compute_piecewise()
        self.str_var = [str(x) for x in R.gens()]
        self.amb_dim = self.sec_fan.ambient_dim()

    def _secondary_fan(self):
        """
        Returns the secondary fan associated with ``self.A``
        For details see the documentation of the gfan software package:
            https://users-math.au.dk/~jensen/software/gfan/gfan.html
        """
        gfan_input = "{" + ", ".join(str(row) for row in self.A.rows()) + "}"
        return PolyhedralFan(gfan(gfan_input, "secondaryfan"))

    def _compute_piecewise(self):
        num_variables = self.A.nrows()
        max_degree = self.sec_fan.lineality_dim()
        ...
        fan_rays = self.sec_fan.rays()
    
        max_cones = []
        quasi_polynomials = []
        for ray_lists in self.sec_fan.maximal_cones().values():
            for ray_list in ray_lists:
                ...
                term_dict = self._get_term_dict(self.A, cone_points)
                ...
                for degree, terms in term_dict.items():
                    cone_poly += T.interpolation(degree, cone_points, terms)*k**degree
    ...
    def _estimate_period(self, chamber):
        ...
        return lcm(a for a in self.A.list() if a)
    ...
    def _create_polytope_from_matrix(self, b):
        inequalities = [[b[k]] + list(-self.A.rows()[k]) for k in range(self.A.nrows())]
    ...
    def _projection_generator(self):
        orth_vectors = []
        for vec in self.sec_fan.fan_dict["ORTH_LINEALITY_SPACE"]:
            orth_vectors.append(free_module_element([int(val) for val in vec.split(" ")]))
    
        lin_vectors = []
        for vec in self.sec_fan.fan_dict["LINEALITY_SPACE"]:
            lin_vectors.append(free_module_element([int(val) for val in vec.split(" ")]))

        change_of_basis_matrix = Matrix(orth_vectors + lin_vectors)
    ...
    def evaluate(self, point):
        ...
        for idx, cone in enumerate(self.max_cones):
            if proj in cone:
                keywords = {x: proj[k] for k, x in enumerate(self.str_var)}
                return self.qps[idx](**keywords)
        return 0
